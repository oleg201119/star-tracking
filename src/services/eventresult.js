...
export default class EventresultService {
	static async getMenuResult(eventID, language) {
		const url = `${API_ENDPOINT}/App/Results/MenuCategories/${eventID}?language=${language}`;
	...
	static async getHeaderResult(eventID, categoryID, language) {
		const url = `${API_ENDPOINT}/App/Results/Header?eventID=${eventID}&categoryID=${categoryID}&language=${language}`;
