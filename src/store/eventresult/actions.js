	...
	return async (dispatch) => {
		try {
			...
			if (eventresult[0].ID === 0 && eventresult[0].ChildCategories === null) {
				const results = await EventresultService.getWinnerResult(eventID, language);
...
export async function fetchWinnerInfo(eventId, categoryId, showNbWinners, language) {
	try {
		const fetchPromises = [
			...
			EventresultService.getBodyResult(eventId, categoryId, 0, showNbWinners, -1, 'ASC', language)
		];
