...
class Resetpwd extends Component {
	...
	componentDidMount() {
		this.setState({ email: this.props.location.state.selectEmail });
		window.scrollTo(0, 0);
	}
	componentWillReceiveProps(nextProps) {
		...
		if (resetPwd === false) {
			this.setState({ buttonState: 'error', loading: false });
					...
					<div className="resetpwd-description">
						...
						{this.state.buttonState !== 'success' ? (
							<div className="resetpwd-body">
								{t(
									'Als u uw paswoord vergeten bent, kunt u Star-Tracking vragen uw paswoord opnieuw in te stellen.'
								)}
								<br />
								{t(
									'Om na te gaan dat u zelf deze aanvraag stuurt, vragen we u eerst een bevestiging per mail.'
								)}
							</div>
						) : (
							<div className="resetpwd-body">
								{t(
									'U zal dan binnen enkele minuten een Star-Tracking email ontvangen met daarin een link.'
								)}
								<br />
								{t(
									'Als u op deze link klikt, zal u uw paswoordwijziging bevestigen en zal uw paswoord opnieuw ingesteld worden.'
								)}
							</div>
						)}
					</div>
				...
				<div className="login-main">
					...
					{this.state.buttonState === 'success' ? (
						...
						<Button
							...
							onClick={() => {
								this.setState({ buttonState: '', loading: true });
